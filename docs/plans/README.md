...
| [Journal/Trips and offline region management — Region management phase](journal-trips-and-offline-regions.md) | Region management section built, **open PR** [#26](https://github.com/slayer8366/Forager/pull/26), branch `claude/plan-implementation-rjzmkr` (see the doc's own "Handoff, 2026-08-20" section for exactly what shipped vs. what's deferred). Multi-region `OfflineMapRepository` (`MIGRATION_4_5`/`OfflineRegionEntity`/`OfflineRegionDao`), per-region Settings list with staleness badges and delete, a deliberately-set tile budget (`setOfflineMapboxTileCountLimit` confirmed to exist on the pinned MapLibre artifact, resolving one of the plan's own open questions), and a DataStore-backed cold-start default. 454 unit tests green, full debug build compiles; nothing here is hardware-verified (no device in this sandbox). Trips, coverage math, the Maps-tab coverage overlay, and privacy/export are still design-only. | PMTiles Worker → real app wiring (merged, above) — this phase builds directly on `MapLibreOfflineMapRepository` |
