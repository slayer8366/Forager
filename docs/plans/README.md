...
| [PMTiles Worker → real app wiring](pmtiles-worker-android-wiring.md) | **Steps 3-4 done, on `claude/offline-maps-integration-21uez7`** (not yet merged as of this writing — check the branch/PR). `MapLibreOfflineMapRepository` replaces `OsmdroidOfflineMapRepository`/`PersistentTileWriter`/`OfflineMapStatusFile` (all deleted) in `AppContainer`; offline downloads now pull real vector tiles from the live Worker via MapLibre's `OfflineManager`, using a glyph-stripped style generated with Protomaps' own `@protomaps/basemaps` package. Steps 1-2 (a live, labeled vector basemap reachable from the real map screen) are **still not done** — deliberately deferred, see the doc's own "Update, 2026-08-19" section for why. Not hardware-verified by this session (no device available); flagged, not assumed working. | PR #23 (mechanism + glyph finding) · PMTiles Worker infra (both above, done) · Phase 1b steps 1-2 wired into the *real* app — still not true, now explicitly deferred rather than just unverified |
