...
directory's own convention, this archive is not itself edited later — a later audit supersedes
an earlier one rather than editing it.

**Why this file exists:** the seven-workstream split, quoted verbatim below exactly as it stood
in `docs/plans/pr26-rework.md` immediately before this rewrite (commit `269692e`, including the
Workstream 1 status block, Workstream 3 inheritance note, and Ownership gap subsection added in
that same commit), is being replaced by a dependency-derived four-workstream split. The split
itself is worth keeping on record, not just its replacement — including why it failed, since the
failure mode is a general one this project wants on file rather than only implicit in a diff.

---

## Why it was superseded

**The topology is a star, not a chain.** The old split cut the work by *activity* — reapply,
renumber, reconcile, correct drift, redesign, capture, comply. The actual code is connected by
*what types what*. A dependency-graph pulse (2026-08-22) found that every boundary-crossing edge
but one terminates at `domain/OfflineMapRepository.kt` — a file the old plan assigned to **no**
workstream by name. Workstream 1 (below) hit this trunk file three separate times (the
`AvailabilityScreen.kt` composables, `AvailabilityUiState.kt`'s offline fields, and
`MapLibreOfflineRegionMetadata.kt`) and closed at roughly a third of its stated scope as a result.

**The breaking-change footprint could not be staged.** `OfflineMapInfo` is deleted outright by
the interface change, not aliased — 16 files on `main` reference it, 13 of those import and
construct it in real code (`MapLibreOfflineMapRepository.kt`, `AvailabilityViewModel.kt`, and 11
test files). `OfflineMapStatus` changes case count, case names, and payload shape in the same
change. Contract and every one of its consumers had to land in one compiling unit; no ordering
of the old seven workstreams avoided this.

**The plan systematically described the smallest visible edit, not the actual compiling unit** —
three separate instances of the same failure shape, found while executing the old split rather
than while reading it:

- **Workstream 3's** text described its own job as five `onError`/`IOException` sites in
  `MapLibreOfflineMapRepository.kt`. It was actually the trunk workstream — the one that had to
  own `domain/OfflineMapRepository.kt` itself, which nothing in its own text named.
- **Workstream 4's** text described its job as "a 15th constructor parameter and one constant."
  The actual diff required rewriting `loadOfflineMapStatus`, `onDownloadOfflineMaps`, and
  `onDeleteOfflineMaps` in `AvailabilityViewModel.kt` in full, plus updating every fake
  `OfflineMapRepository` implementation across 12+ test files.
- **Workstream 1's** text described porting three composables (`OfflineMapsPanel`,
  `OfflineRegionsSection`, `OfflineMapStatusContent`) as largely-as-is. They weren't portable in
  isolation — two of the three already existed on `main` as shipped, differently-shaped code, and
  all three type directly against declarations in the unowned trunk file.

**The corrected cause, which matters more than any one of the three examples above:** the
original author was not working from memory or skipping verification. The plan's own preamble
cites a real, dated pulse (2026-08-22, `claude/plan-implementation-rjzmkr` read at `main` =
`3b6021b`) as its evidence base, and that pulse's answers — file paths, line counts, quoted code —
were accurate as far as they went. **The failure was in what the pulse was asked: it answered
"what does #26 change" (file-level) when the plan needed "what types against what" (type-level).**
A file-level answer to a type-level question looks complete — every file PR #26 touches really
was accounted for — and isn't, because it can't see which of those files' declarations other
files depend on. This is the general failure mode worth recording, independent of this specific
plan: a pulse's scope has to match the shape of the question the plan is actually going to ask
of it.

**One prior document came close.** `docs/audits/2026-08-23-session-handoff.md`'s own phase stack
modeled "Workstreams 2-7" as a single unit blocked on Workstream 1 — not seven independent
lines. That's evidence the original author sensed real coupling between the workstreams. What
that model got wrong was the shape: it treated the coupling as a **chain** (everything waits on
WS1, then proceeds workstream by workstream), when the dependency graph shows a **star** (nearly
everything depends directly on one trunk file, not on each other in sequence).

---

## The seven workstreams, verbatim, as they stood before this rewrite
