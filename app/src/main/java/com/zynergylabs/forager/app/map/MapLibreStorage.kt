 ...
 * the real [com.zynergylabs.forager.app.ForagerApplication] for every unit test, and no test exercises either real
 ...
 *
 * This exists because one entry point didn't call the redirect first: `com.zynergylabs.forager.app.ui.map.SightingsMap`'s
 * own `MapLibre.getInstance(context)` call used to reach the native library directly, with nothing
 * before it establishing where the resource database lives. `com.zynergylabs.forager.app.map.MapLibreOfflineMapRepository`'s
 ...
 * [com.zynergylabs.forager.app.ui.map.SightingsMap] and [com.zynergylabs.forager.app.map.MapLibreOfflineMapRepository].
 */
