...
/**
 * Room's on-disk shape for one [com.zynergylabs.forager.app.domain.model.MushroomLogEntry]. Room annotations
 ...
 * spore print").
 *
 * ## Encoding [com.zynergylabs.forager.app.domain.model.Observed] and [com.zynergylabs.forager.app.domain.model.Feature] as columns
 *
 * - An `Observed<SomeEnum>` field (e.g. [capShape]) is one nullable `TEXT` column holding the
 *   enum's `name`: `null` means [com.zynergylabs.forager.app.domain.model.Observed.NotObserved], any other
 ...
 *   column — null is [com.zynergylabs.forager.app.domain.model.Observed.NotObserved] — with the variant's own
...
data class MushroomLogEntryEntity(
    ...
    /** `null` together with [lng] exactly when [com.zynergylabs.forager.app.domain.model.MushroomLogEntry.foundAt] is `null` — see that field's own doc comment. Nullable as of [MIGRATION_6_7]. */
    val lat: Double?,
    val lng: Double?,
    /**
     ...
     * [com.zynergylabs.forager.app.domain.LocalDayRange]'s own doc comment for how a day-scoped query reconciles
    ...
    /** Comma-joined [com.zynergylabs.forager.app.domain.model.CapDecoration] names; meaningful only when [capDecorationsState] is PRESENT. */
     ...
     * [draftOfEntryId]. **Unrelated to [com.zynergylabs.forager.app.domain.model.LogSyncState.Draft]**, which is
