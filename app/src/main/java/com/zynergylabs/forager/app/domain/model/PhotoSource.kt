 ...
 * result type the same way [com.zynergylabs.forager.app.domain.LocationProvider] already covers
 * `android.location`. The real, `Uri`-carrying implementations (`CameraCapturePhotoSource`,
 * `GalleryImportPhotoSource`) live in `com.zynergylabs.forager.app.photo`, alongside the
 * [com.zynergylabs.forager.app.domain.PhotoStore] implementation that's the only thing that ever inspects one.
 *
 ...
 * the device?* [FilePhotoStore.persist] and [com.zynergylabs.forager.app.ui.log.MushroomLogViewModel] both
