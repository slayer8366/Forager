 ...
 * `com.zynergylabs.forager.app.map.MapLibreOfflineMapRepository`) — real device/SDK behavior this dispatch's own
 ...
 * query against [com.zynergylabs.forager.app.data.local.OfflineRegionEntity], the same table
 ...
 * all (see [com.zynergylabs.forager.app.data.local.OfflineRegionEntity]'s own doc comment). Reporting a fabricated
...
interface OfflineRegionDayIndex {
    /**
     * Every region *downloaded* on one local day — [com.zynergylabs.forager.app.data.local.OfflineRegionEntity.createdAtEpochMillis],
