...
object MotionTokens {
    ...
    val feedbackMotionSpec: FiniteAnimationSpec<Float> = tween(durationMillis = FEEDBACK_MOTION_DURATION_MS, easing = EaseOut)
    ...
    val narrativeRevealSpec: FiniteAnimationSpec<Float> = tween(durationMillis = NARRATIVE_REVEAL_DURATION_MS, easing = EaseOut)

    // §2 "Map markers": soft scale + fade entrance when density/performance allow.
    const val MARKER_ENTRANCE_DURATION_MS: Int = 250
    val markerEntranceSpec: FiniteAnimationSpec<Float> = tween(durationMillis = MARKER_ENTRANCE_DURATION_MS, easing = EaseOut)
    ...
    val selectionPulseHalfCycleSpec: FiniteAnimationSpec<Float> =
    ...
    val locationIndicatorMoveSpec: FiniteAnimationSpec<Float> =
    ...
    val routeRecalculationMorphSpec: FiniteAnimationSpec<Float> =
    ...
    val panelMotionSpec: FiniteAnimationSpec<Float> = tween(durationMillis = PANEL_MOTION_DURATION_MS, easing = EaseOut)
