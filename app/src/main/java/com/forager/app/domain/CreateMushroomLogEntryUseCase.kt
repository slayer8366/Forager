 ...
 * persists it immediately. Unlike [com.forager.app.domain.SavePlannedTripUseCase], there's no
 * required field to validate first, since every section starts as
 * [com.forager.app.domain.model.Observed.NotObserved]/[com.forager.app.domain.model.Feature.NotObserved]
 * by design (see [MushroomLogEntry.draft]). Persisting immediately, rather than only once the
 * forager finishes filling it in, is what makes the deferred-observation edit flow possible: the
 * entry exists in storage from the moment it's started, ready to be reopened and completed later.
 *
