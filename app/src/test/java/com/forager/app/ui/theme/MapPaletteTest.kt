 ...
 * either — see the removed-test note ahead of the mark-on-mark checks below for why, and what the
 * checks asserted for whoever revives them. What still runs for night: [MapPalette.NIGHT_WARM]
 * and [MapPalette.NIGHT_INK] contrast against *each other* (the mark-on-mark bar), and that each
 * sits on the expected side of every day marker's lightness — legibility against the ground
 * itself is a hardware question for now, not a colour-arithmetic one, since it comes from
 * `SightingsMap.kt`'s icon halo rather than tile contrast (`BasemapStyles.kt`'s
 * `NIGHT_RASTER_PAINT` doc comment, "Dimming removed").
 *
 * ## What this does not establish
 *
 * Legibility against *real tiles*, night or day, or that the night icon shapes and halos actually
 * read as distinct from each other and from the ground on a real screen. The day tile reference is
 * modelled, not sampled. These checks bound regressions against a stated reference; the README's
 * "Not yet verified" section is where the hardware question lives. A green run here is not
 * evidence that the map reads well, day or night.
 */
class MapPaletteTest {
    ...
    /**
     * There used to be two tests here checking [MapPalette.NIGHT_WARM]'s contrast against
     * [MapPalette.NIGHT_TILE_REFERENCE]. Removed rather than `@Ignore`d: this project's CI
     * (`.github/workflows/ci.yml`, "Summarize the test results") fails the whole build on *any*
     * skipped test, by design, so `@Ignore` is not a quiet way to keep dead-but-useful assertions
     * around here the way it might be elsewhere.
     *
     * Kept here as a note instead, for whoever revives them once colour inversion (the deferred
     * night-mode replacement — `docs/plans/map-redesign.md`, "Deferred: night-mode colour
     * inversion") gives night a dark ground again and [MapPalette.NIGHT_WARM] is retuned for it:
     *
     * ```
     ...
     * assert(ratio >= 4.0)
     *
     * // A ratchet: night must be no less legible against its ground than day's weakest marker
     * // is against day's ground.
     * val worstDay = markers(MapPalette.DAY).values.minOf { contrastRatio(it, MapPalette.DAY_TILE_REFERENCE) }
     * assert(ratio >= worstDay)
     * ```
     *
     * Both would fail today, for an expected reason, not a bug: since `BasemapStyles.kt` stopped
     * dimming the night ground ("Dimming removed, 2026-08-26" in that file's `NIGHT_RASTER_PAINT`
     * doc comment), [MapPalette.NIGHT_TILE_REFERENCE] equals [MapPalette.DAY_TILE_REFERENCE] — a
     * light tile — and [MapPalette.NIGHT_WARM] is itself a light colour, never retuned for that
     * ground (night-mode legibility currently comes from the icon halo `SightingsMap.kt`'s
     * `*Bitmap` functions draw, not from tile contrast). Measured: ≈1.26:1, well under both the
     * 4.0:1 floor and day's weakest marker (1.79:1).
     */
