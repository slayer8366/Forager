 ...
 * composed under Robolectric (see [com.zynergylabs.forager.app.ui.map.SightingsMapOverlayDataTest]'s own doc
 * comment for why: native MapLibre calls), so this drives the screen through a capturing stub
 * [MapSlot] — the same pattern [com.zynergylabs.forager.app.ui.map.CentrePinLocationPickerTest] already
...
class CartographyEntryReportScreenMapTest {
    ...
    private var capturedRegion: com.zynergylabs.forager.app.domain.model.Region? = null
     ...
     * [com.zynergylabs.forager.app.domain.GetCartographyEntryMapDataUseCase]'s own test for that filtering
    ...
    private fun offlineRegionSummary(): OfflineRegionSummary = OfflineRegionSummary(
        id = 1L,
        name = "Ridge Region",
        region = com.zynergylabs.forager.app.domain.model.Region(lat = 45.5, lng = -122.5, radiusKm = 10),
