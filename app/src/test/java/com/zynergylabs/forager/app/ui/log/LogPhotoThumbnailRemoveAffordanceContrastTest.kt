 ...
 *
 * Pure arithmetic, no Robolectric or composition — same shape as [com.zynergylabs.forager.app.ui.theme.ThemeContrastTest],
...
class LogPhotoThumbnailRemoveAffordanceContrastTest {
    ...
    /** [Bark] — this theme's own explicit `scrim` value in both color schemes, per [com.zynergylabs.forager.app.ui.theme.ThemeCompletenessTest]'s requirement that every role the UI reads be intentional, not a Material baseline passthrough. */
    private val scrim = Bark
    private val glyph = Color.White

    /** WCAG 2.2 SC 1.4.11, non-text contrast — the same bar [com.zynergylabs.forager.app.ui.theme.ThemeContrastTest] applies to icon/control-shaped elements. */
