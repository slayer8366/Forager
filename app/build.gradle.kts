...
android {
    ...
    // 2026-09-09 asserted one it does not own. It appears nowhere in this repository -- not in the
    // tree and not in its history, which is why this is a new repository rather than a rewrite of
    // the old one. There is deliberately no CI gate matching it: a gate has to spell the term in
    // order to match it, and carrying one would put the string back into this repository's first
    // commit permanently. The exposure was the domain claim only -- the app is still called Forager,
    // and the Play listing, the privacy policy and the GPX namespace URI were all already correct
